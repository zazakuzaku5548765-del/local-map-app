...
const expiryOptions:{value:ExpiryDays;label:string;description:string}[]=[{value:null,label:'期限なし',description:'継続して掲載'},{value:30,label:'30日',description:'30日後に非表示'},{value:60,label:'60日',description:'60日後に非表示'},{value:90,label:'90日',description:'90日後に非表示'}]
...
const photoUploadErrorMessage=(error:unknown)=>{const message=error instanceof Error?error.message.toLowerCase():'';if(message.includes('row-level security')||message.includes('unauthorized')||message.includes('403'))return '写真保存の権限エラーが発生しました。Storageのポリシーを確認してください。';if(message.includes('bucket')||message.includes('not found'))return '写真の保存先が見つかりません。Storageの設定を確認してください。';return '写真をアップロードできませんでした。通信状況を確認してもう一度お試しください。'}
export default function App(){
  ...
  const submit=async(data:{category:Category;body:string;occurredAt:string|null;occurredPeriod:OccurredPeriod;sourceType:SourceType;files:File[];placeName:string;expiryDays:ExpiryDays})=>{
    ...
    const saved=await savePost({placeId:selectedPlace?.id,latitude:point.lat,longitude:point.lng,placeName:data.placeName||null,address:selectedPlace?.address||null,category:data.category,content:data.body,occurredAt:data.occurredAt,occurredPeriod:data.occurredPeriod,sourceType:data.sourceType,expiryDays:data.expiryDays})
    let photoError=''
    if(data.files.length){try{await uploadPostImages(saved,data.files)}catch(error){console.error('Photo upload failed',error);photoError=photoUploadErrorMessage(error)}}
    trackEvent('post_complete',{place_id:saved.placeId,categories:data.category,source_type:data.sourceType})
    await refresh();setComposer(false);setDraftPoint(null);setToast(photoError?`投稿本文は保存されました。${photoError}`:'投稿を公開しました');setTimeout(()=>setToast(''),photoError?7000:3600)
  }
...
function Composer({place,point,nearby,onUseNearby,close,submit}:{place:Place|null;point:{lat:number;lng:number}|null;nearby:{place:Place;distance:number}[];onUseNearby:(place:Place)=>void;close:()=>void;submit:(d:{category:Category;body:string;occurredAt:string|null;occurredPeriod:OccurredPeriod;sourceType:SourceType;files:File[];placeName:string;expiryDays:ExpiryDays})=>Promise<void>}){
 const [step,setStep]=useState(1),[category,setCategory]=useState<Category|null>(null),[body,setBody]=useState(''),[occurredPeriod,setOccurredPeriod]=useState<OccurredPeriod>('recent'),[occurredDate,setOccurredDate]=useState(''),[sourceType,setSourceType]=useState<SourceType|null>(null),[files,setFiles]=useState<File[]>([]),[expiryDays,setExpiryDays]=useState<ExpiryDays>(null),[placeName,setPlaceName]=useState(''),[accepted,setAccepted]=useState(false),[warning,setWarning]=useState(''),[submitting,setSubmitting]=useState(false),[submitError,setSubmitError]=useState('')
 ...
 {step===5&&<><p className="eyebrow">STEP 5 — 写真と掲載期間</p><h2>写真と掲載期間を<br/>設定できます</h2><PhotoPicker files={files} setFiles={setFiles}/><h3 className="form-subtitle expiry-title">掲載期間</h3><div className="expiry-grid">{expiryOptions.map(x=><button key={x.label} className={expiryDays===x.value?'selected':''} onClick={()=>setExpiryDays(x.value)}><Clock3/><span><b>{x.label}</b><small>{x.description}</small></span>{expiryDays===x.value&&<Check/>}</button>)}</div><p className="microcopy">期限が来ると地図や投稿一覧から非表示になります。投稿データは削除されません。</p></>}
 {step===6&&<><p className="eyebrow">STEP 6 — 確認</p><h2>この内容で投稿しますか？</h2><div className="review-box"><small>{place?.name||placeName||'選択した地点'} ・ {category}</small><p>{body}</p><span>発生時期：{occurredPeriod==='date'&&occurredDate?`${new Date(`${occurredDate}T00:00:00`).toLocaleDateString('ja-JP')}頃`:periodOptions.find(x=>x.value===occurredPeriod)?.label}</span><span>情報源：{sourceOptions.find(x=>x.value===sourceType)?.label} ・ 写真{files.length}枚</span><span>掲載期間：{expiryOptions.find(x=>x.value===expiryDays)?.label}</span><span>投稿日：投稿完了時に自動記録</span></div><label className="consent"><input type="checkbox" checked={accepted} onChange={e=>setAccepted(e.target.checked)}/><span><b>投稿前の最終確認</b>個人を特定する情報や誹謗中傷を含まず、自分が確認した情報であることを確認しました。</span></label>{submitError&&<div className="submit-error"><AlertCircle/>投稿できませんでした。通信状況を確認してもう一度お試しください。</div>}</>}
 </div><footer>{step<6?<button className="primary wide" disabled={!canNext} onClick={()=>{if(step===3&&!check())return;setStep(step+1)}}>次へ進む</button>:<button className="primary wide" disabled={!accepted||Boolean(warning)||submitting} onClick={async()=>{if(!category||!sourceType)return;setSubmitting(true);setSubmitError('');try{await submit({category,body,occurredAt:occurredPeriod==='date'?occurredDate:null,occurredPeriod,sourceType,files,placeName,expiryDays})}catch(error){console.error('Post submission failed',error);setSubmitError('failed');setSubmitting(false)}}}>{submitting?<><LoaderCircle className="spin"/>投稿しています…</>:'投稿する'}</button>}</footer></section></div>
}
...
function PhotoPicker({files,setFiles}:{files:File[];setFiles:(files:File[])=>void}){
 ...
 const add=(selected:File[])=>{try{const next=[...files,...selected];validateImageFiles(next);setFiles(next);setError('')}catch(reason){setError(reason instanceof Error?reason.message:'写真を追加できませんでした')}}
 return <div className="photo-picker"><label className="upload photo-upload"><ImagePlus/><b>写真を追加（最大3枚）</b><small>スマートフォンの写真ライブラリまたはPCから選べます。送信前に自動圧縮します。</small><span className="photo-select-button">写真を選択</span><input type="file" accept="image/*" multiple disabled={files.length>=3} onChange={e=>{add(Array.from(e.target.files||[]));e.currentTarget.value=''}}/></label>{previews.length>0&&<div className="photo-previews">{previews.map((src,index)=><figure key={src}><img src={src} alt={`選択した写真 ${index+1}`}/><button type="button" aria-label={`写真${index+1}を削除`} onClick={()=>setFiles(files.filter((_,i)=>i!==index))}><Trash2/></button></figure>)}</div>}{error&&<div className="photo-error"><AlertCircle/>{error}</div>}<small className="photo-count">{files.length} / 3枚</small></div>
}

function PlaceDetail({place,posts,onBack,onPost,onReport,toast}:{place:Place;posts:Post[];onBack:()=>void;onPost:()=>void;onReport:(id:string)=>void;toast:string}){return <main className="detail-page"><header><button onClick={onBack}><ChevronLeft/></button><span>地点の情報</span><button><Search/></button></header><section className="detail-hero"><div className="mini-map"><MapPin/></div><p className="eyebrow">地域のみなさんからの情報</p><h1>{place.name}</h1><p className="address"><MapPin size={15}/>{place.address}</p><div className="stats"><div><b>{posts.length}</b><span>投稿</span></div><div><b>{new Set(posts.map(p=>p.sourceType)).size}</b><span>情報源の種類</span></div><div><b>{posts.reduce((n,p)=>n+p.helpful,0)}</b><span>参考になった</span></div></div></section><section className="detail-content"><div className="section-title"><div><small>みんなの声</small><h2>この場所の投稿</h2></div><button onClick={onPost}><Plus/>情報を追加</button></div>{posts.map(p=><article className="post-card" key={p.id}><header><span>{p.category}</span><time>投稿日：{new Date(p.createdAt).toLocaleDateString('ja-JP')}</time></header>{p.imageUrls.length>0&&<div className={`post-images count-${p.imageUrls.length}`}>{p.imageUrls.map((url,index)=><img key={url} src={url} alt={`投稿写真 ${index+1}`} loading="lazy"/>)}</div>}<p>{p.body}</p><div className="event-time">発生時期：{periodLabel(p)}{p.expiresAt&&<> ・ 掲載期限：{new Date(p.expiresAt).toLocaleDateString('ja-JP')}</>}</div><div className="post-meta"><span><ShieldCheck/> {p.verification.join('・')}</span><button onClick={()=>onReport(p.id)}><Flag/>通報</button></div></article>)}{!posts.length&&<div className="detail-empty"><MapPin/><b>まだ情報がありません</b><p>この場所について知っていることを、最初に共有しませんか？</p></div>}</section><button className="post-fab detail-fab" onClick={onPost}><Plus/>この場所に投稿する</button>{toast&&<div className="toast"><Check/>{toast}</div>}</main>}
